...
**Seed layout**: **constant across generations**, unlike the GA's per-generation rotation. Training seeds are `(seed + 1) * 9973` and held-out θ-eval seeds are `(seed + 1000) * 9973`, both fixed for the whole run. This is the Salimans et al. 2017 convention and it is load-bearing: the ES gradient estimator `g[k] = (1/(N·σ)) · Σᵢ Rᵢ·εᵢ[k]` assumes every perturbation's reward is drawn from the same distribution, so rotating game seeds between generations mixes "θ improved" with "games got harder/easier" and the optimizer chases seed noise instead of real improvement. Constant seeds give a clean gradient and a held-out curve that is a genuine "objective value at θ" rather than a randomized re-eval. Trade-off: θ can in principle overfit to the fixed slate — in practice the 48 × games_per_chart × 5 charts × len(providers) effective sample size per generation makes that a non-issue within a 30-gen run.
